...
test('color grading preview exposes split compare and scopes', async ({ page }) => {
	...
	await setRangeValue(inspector.getByLabel('Look intensity'), 0)
	await inspector.getByRole('button', { name: 'Apply look Golden' }).click()
	await setRangeValue(inspector.getByLabel('Look intensity'), 100)
	await expect(renderer.locator('.ve-renderer__layer--video').first()).toHaveCSS('filter', /brightness\(1\.08\).*sepia\(0\.0875\).*hue-rotate\(5deg\)/)
	await setRangeValue(inspector.getByLabel('Look intensity'), 0)
	await inspector.getByRole('button', { name: 'Apply look Mono' }).click()
	await expect(inspector.getByText('Mono 100%')).toBeVisible()
	await expect(renderer.locator('.ve-renderer__layer--video').first()).toHaveCSS('filter', /saturate\(0\)/)
	await inspector.getByRole('button', { name: 'Apply look Cinema' }).click()
	await setRangeValue(inspector.getByLabel('Look intensity'), 50)
	...
	await expect(inspector.getByLabel('Look intensity')).toBeDisabled()
	...
	await expect(renderer.locator('video')).toHaveCount(1)
	await expect(renderer.locator('.ve-renderer__compare-before canvas.ve-renderer__before-snapshot')).toHaveCount(1)
	await expect(renderer.locator('.ve-renderer__compare-before video')).toHaveCount(0)
	const splitResizeBox = await resizeHandle.boundingBox()
	if (!splitResizeBox) {
		throw new Error('Preview/inspector resize handle is unavailable in split compare')
	}
	await page.mouse.move(splitResizeBox.x + splitResizeBox.width / 2, splitResizeBox.y + splitResizeBox.height / 2)
	await page.mouse.down()
	await page.mouse.move(splitResizeBox.x + splitResizeBox.width / 2 + 70, splitResizeBox.y + splitResizeBox.height / 2)
	await page.mouse.up()
	await expect.poll(async () => renderer.locator('.ve-renderer__compare-before canvas.ve-renderer__before-snapshot').evaluate((canvas) => {
		const element = canvas as HTMLCanvasElement
		const rect = element.getBoundingClientRect()
		return element.width === Math.round(rect.width) && element.height === Math.round(rect.height)
	})).toBe(true)
