...
- Executor: render + dispatch progress updates → on completion dispatch result update
- Component: читает exportProgress state field → auto-download при stage==='done'
- Нет promise, нет callback, нет runtime refs

**1. Модель: ExportProgress state field**
...
interface ExportProgress {
  ...
  result?: {
    downloadUrl: string    // local blob URL (non-serializable)
    fileName: string       // для download
    frameCount: number     // metadata for UI
    size: number          // metadata for UI
    // blob НЕ хранится в state (живет в closure executor или DI)
  }
}
```

**2. DKT Actions на SessionRoot/Clip:**

Добавить на SessionRoot:
```ts
{
  ...
  steps: [
    ...
    },
    {
      ...
      input: { exportPlan: '$output.clipSources', fps: '$output.fps', ... }
    }
  ]
  // Action заканчивается после того, как $fx_renderExport встанет в очередь
  // Результат приходит через dispatch('setExportProgress', { stage: 'done', result: {...} })
}
...
{
  type: 'requestSelectedClipExport',  // root action
  deps: 'activeProject | selectedClip',
  steps: [
    { target: 'selectedClip', action: 'requestClipExport' }
    // OR: inline delegation logic
  ]
}

{
  type: 'setExportProgress',
  input: { stage, progress?, message?, result? },
  impl: (ctx, input) => {
    ctx.attrs.exportProgress = input
  }
}
```

**3. Runtime task executor ($fx_renderExport):**

Executor реализует:
```ts
{
  async *execute(message) {
    const { exportPlan, range, rootScope } = message.input
    
    // Initial state: queued
    yield { type: 'dispatch', action: 'setExportProgress', payload: { stage: 'queued', progress: 0 } }
    
    try {
      // Render with progress callback
      const result = await env.export.render({ plan, range }, (event) => {
        // Dispatch progress update
        yield { type: 'dispatch', action: 'setExportProgress', payload: { stage: 'rendering', progress: event.progress } }
      })
...
**4. UI Components: Read state, auto-download**
...
useEffect(() => {
  if (exportProgress?.stage === 'done' && exportProgress.result) {
    // Auto-download
    const link = document.createElement('a')
    link.href = exportProgress.result.downloadUrl
    link.download = exportProgress.result.fileName
    link.click()
  }
...
const handleExportProject = () => {
  dispatchRoot('requestProjectExport')
}

// Same auto-download effect as InspectorExportTabPanel
```

**5. Удалить старый код:**
- Удалить `queueExport` helper функцию
- Удалить `buildFallbackExportPlan` (больше не нужна без fallback)
- Удалить методы: `queueProjectExport`, `queueClipExportById`, `queueSelectedClipExport` из adapter
- Удалить поле `downloadUrl` из `ExportRenderResult` (результат теперь в state)

**6. Ключевые отличия от старого подхода:**
- ✅ Нет callbacks в payload → serializable
- ✅ Нет promise → pure DI message passing
- ✅ Progress в state field → синхронизируется через P2P
- ✅ Auto-download в UI useEffect → реагирует на state
- ✅ Blob URL управляется executor/DI, не передается через state
