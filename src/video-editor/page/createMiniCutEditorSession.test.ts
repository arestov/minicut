...
const mockState = vi.hoisted(() => {
	...
	return {
		...
		createDefaultRtcConfig: vi.fn((iceServer: RTCIceServer | null) => ({ iceServers: iceServer ? [iceServer] : [] })),
...
describe('createMiniCutEditorSession', () => {
	...
	test('creates a weather-style session facade with compact debug helpers', async () => {
		const { createMiniCutEditorSession } = await import('./createMiniCutEditorSession')
		const session = createMiniCutEditorSession()

		session.bootstrap()
		session.dispatchAction('setCursor', 1.25, null)

		expect(mockState.runtime.bootstrap).toHaveBeenCalledWith({ sessionKey: 'minicut-local' })
		expect(mockState.runtime.dispatchAction).toHaveBeenCalledWith('setCursor', 1.25, null)
	...
	test('uses the room id as DKT session key when P2P signal URL is enabled', async () => {
		...
		expect(mockState.runtime.bootstrap).toHaveBeenCalledWith({ sessionKey: 'room-alpha' })
		expect(mockState.createBrowserHarnessPlatform).toHaveBeenCalledWith(expect.objectContaining({
