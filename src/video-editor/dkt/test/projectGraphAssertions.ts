...
export const expectProjectGraphInvariants = async (
	ctx: DktTestContext,
): Promise<void> => {
	const activeProject =
	...
	for (const clip of flatClips) {
		...
		if (mediaKind !== "text" && resourceRel.length > 0) {
			...
			expect(
				resources.some(
					(resource) => resource._node_id === resourceModel._node_id,
				),
			).toBe(true);
		...
		if (typeof clipRenderData?.resourceId === "string") {
			expect(
				resources.some(
					(resource) => resource._node_id === clipRenderData.resourceId,
				),
			).toBe(true);
		}
...
export const readProjectGraph = async (ctx: DktTestContext) => {
	...
	const resources = await ctx.queryRel(activeProject, "resources");
	const clips = (
		await Promise.all(tracks.map((track) => ctx.queryRel(track, "clips")))
	).flat();
...
export const findTrackByKind = async (
	ctx: DktTestContext,
	kind: "video" | "audio",
) => {
