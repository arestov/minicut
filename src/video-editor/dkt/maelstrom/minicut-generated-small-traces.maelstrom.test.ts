...
describe("MiniCut maelstrom generated small traces", () => {
	...
	for (const profile of createMiniCutMaelstromProfiles()) {
		for (const seed of seeds) {
			it(`keeps timing conflict summaries stable across delivery variants seed=${seed} with ${profile.name}`, async () => {
				await runMiniCutMetamorphicTrace(generateMiniCutSmallTrace(seed), profile);
			});
		}
	}
});
