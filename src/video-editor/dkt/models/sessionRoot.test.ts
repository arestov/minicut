...
describe("SessionRoot: createProject behavior contract", () => {
	it("createProject sets activeProject and creates two default tracks", async () => {
		...
		expect(project._node_id).toBe(
			ctx.getAttr(ctx.sessionRoot, "activeProjectId"),
		);
	...
	it("handleInit is idempotent after the first init pass", async () => {
		...
		const projectCountAfterFirstInit = (
			await ctx.queryRel(ctx.appModel, "project")
		).length;

		await ctx.lockToRead(async () => {
			await ctx.sessionRoot.dispatch("handleInit");
		});
		const projectCountAfterSecondInit = (
			await ctx.queryRel(ctx.appModel, "project")
		).length;

		expect(projectCountAfterSecondInit).toBeGreaterThanOrEqual(
			projectCountAfterFirstInit,
		);
	});
