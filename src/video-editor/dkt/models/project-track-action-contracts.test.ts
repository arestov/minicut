...
describe("Project action contracts", () => {
	it("importResource creates only a resource when the timeline is empty", async () => {
		...
		const audioTrack = tracks.find(
			(track) => ctx.getAttr(track, "kind") === "audio",
		);
	...
	it("renameProject, setProjectFormat, and setProjectDuration update project attrs", async () => {
		...
		expect(harness.ctx.getAttr(harness.project, "title")).toBe(
			"Renamed Coverage Project",
		);
	...
	it("importResource creates a resource and addResourceToTimeline routes it to the video track", async () => {
		...
		const projectVideoResource = resources.find(
			(resource) =>
		...
		const createdClip = clips
			.filter(
				(clip) => String(clip._node_id) !== String(harness.videoClip._node_id),
			)
			.sort(
				(a, b) =>
					Number(harness.ctx.getAttr(b, "start")) -
	...
	it("addTextClipToVideoTrack creates a text clip and a text node in the root graph", async () => {
		...
		const createdTextClipRel = await harness.ctx.queryRel(
			createdTextNode,
			"clip",
		);
		...
		expect(
			textModels.some(
				(text) => harness.ctx.getAttr(text, "content") === "Project text",
			),
		).toBe(true);
		await expectProjectGraphInvariants(harness.ctx);
	});
...
describe("Track action contracts", () => {
	it("renameTrack, setTrackMuted, and setTrackLocked update track attrs", async () => {
		...
		expect(harness.ctx.getAttr(harness.videoTrack, "name")).toBe(
			"Primary Video",
		);
	...
	it("setClips replaces the clip relation list in the requested order", async () => {
		...
		const trackClipIds = await readNodeIds(
			harness.ctx,
			harness.videoTrack,
			"clips",
		);
	...
	it("moveClipToTrack moves a clip between tracks as a single project graph mutation", async () => {
		...
		expect(
			await readNodeIds(harness.ctx, harness.videoTrack, "clips"),
		).not.toContain(clipId);
		expect(
			await readNodeIds(harness.ctx, harness.audioTrack, "clips"),
		).toContain(clipId);
		expect(await readNodeIds(harness.ctx, harness.videoClip, "track")).toEqual([
			targetTrackId,
		]);
		await expectProjectGraphInvariants(harness.ctx);
	});
