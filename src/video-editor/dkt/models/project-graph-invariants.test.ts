...
describe("project graph invariants", () => {
	it("stay intact after import, append, embedded audio, and split actions", async () => {
		...
		const videoResource = resources.find(
			(resource) => ctx.getAttr(resource, "name") === "Graph Video",
		);
		const audioResource = resources.find(
			(resource) => ctx.getAttr(resource, "name") === "Graph Audio",
		);
		...
		const beforeSplitVideoClipCount = (await ctx.queryRel(videoTrack, "clips"))
			.length;

		const videoTrackClips = await ctx.queryRel(videoTrack, "clips");
		const videoClip = videoTrackClips.find((clip) => {
			const renderData = ctx.getAttr(clip, "clipRenderData") as {
				resourceId?: unknown;
			} | null;
