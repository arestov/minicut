...
describe('Clip action contracts', () => {
	it('rename, color, updateOpacity, and setMediaKind update clip attrs', async () => {
		...
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'rename', 'Renamed Video Clip')
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'color', '#ff00ff')
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'updateOpacity', { opacityPercent: 75 })
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'setMediaKind', 'image')

		expect(harness.ctx.getAttr(harness.videoClip, 'name')).toBe('Renamed Video Clip')
		expect(harness.ctx.getAttr(harness.videoClip, 'color')).toBe('#ff00ff')
		expect(harness.ctx.getAttr(harness.videoClip, 'opacity')).toEqual({ value: 0.8 })
	...
	it('setClipAttrs, setFade, setAudio, setTimelineAttrs, and setTransform patch the clip state', async () => {
		...
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'setClipAttrs', {
			...
			audio: { gain: 1, pan: 0 },
			opacity: { value: 0.6 },
			transform: { x: { value: 1 }, y: { value: 2 }, scale: { value: 1 }, rotation: { value: 0 } },
		})
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'setFade', { edge: 'in', delta: 0.4 })
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'setAudio', { gain: 0.8, pan: -0.25 })
		await dispatchAndSettle(harness.ctx, harness.videoClip, 'setTimelineAttrs', {
			start: 4,
			in: 2,
			duration: 6,
			fadeIn: 0.3,
		...
		expect(harness.ctx.getAttr(harness.videoClip, 'name')).toBe('Baseline Clip')
		...
		expect(harness.ctx.getAttr(harness.videoClip, 'audio')).toEqual({ gain: 0.8, pan: -0.25 })
		expect(harness.ctx.getAttr(harness.videoClip, 'opacity')).toEqual({ value: 0.6 })
	...
	it('setResource, setText, setTrack, setProject, addEffect, removeEffect, reorderEffect, and setEffects mutate relations', async () => {
		...
		await dispatchAndSettle(harness.ctx, clip, 'setResource', { resource: harness.imageResource })
