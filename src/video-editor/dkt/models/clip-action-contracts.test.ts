...
const isTenthsAligned = (value: number): boolean => Math.abs(value * 10 - Math.round(value * 10)) < 1e-9
...
describe('clip action reducer contracts', () => {
	it('moveBy keeps the clip start normalized and non-negative', () => {
		...
		expect(reduceTimelineMoveByAction({ delta: -1.2 }, { start: 0.2 })).toEqual({ start: 0 })
	})
	...
	it('trim keeps the clip end stable on start-edge edits and floors duration at 0.5', () => {
		...
		for (let index = 0; index < 20; index += 1) {
			...
			expect(nextStart + nextDuration).toBeCloseTo(attrs.start + attrs.duration, 6)
			expect(nextIn - attrs.in).toBeCloseTo(nextStart - attrs.start, 6)
		}

		expect(reduceTimelineTrimAction({ edge: 'end', delta: -10 }, { start: 0, in: 0, duration: 1 })).toEqual({ duration: 0.5 })
	})
	...
	it('splitAt only accepts in-bounds split points', () => {
		...
		expect(reduceTimelineSplitAtAction({ time: 2.5 }, attrs)).toEqual({ duration: 1 })
	})

	it('opacity update rounds to tenths and rejects invalid input', () => {
		expect(reduceClipUpdateOpacityAction({ opacityPercent: 87.4 })).toEqual({ opacity: { value: 0.9 } })
		expect(reduceClipUpdateOpacityAction(12.2)).toEqual({ opacity: { value: 0.1 } })
