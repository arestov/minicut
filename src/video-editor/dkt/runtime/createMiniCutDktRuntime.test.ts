...
const findModel = (state: Awaited<ReturnType<ReturnType<typeof createMiniCutDktTestRuntime>['debugDumpAppState']>>, modelName: string) => {
	const matches = [
		...(state?.runtimeModels ?? []),
		...(state?.lined ?? []),
...
const findModelByAttr = (
	state: Awaited<ReturnType<ReturnType<typeof createMiniCutDktTestRuntime>['debugDumpAppState']>>,
	modelName: string,
	attrName: string,
...
const waitForModelAttr = async (
	runtime: ReturnType<typeof createMiniCutDktTestRuntime>,
	modelName: string,
	attrName: string,
	expectedValue: unknown,
) => {
...
describe.skip('createMiniCutDktTestRuntime', () => {
	it('does not boot while disabled', async () => {
		const runtime = createMiniCutDktTestRuntime()
