...
export const LookBrowser = ({
	...
	const activeLook =
		lookPresets.find((look) => look.id === activeLookId) ?? null;
		...
		<section className="ve-look-browser" aria-label="Look Browser">
			<div className="ve-look-browser__header">
				...
				<span className="ve-status-pill">
					{activeLabel}{" "}
					{activeLook ? `${Math.round(clampedIntensity * 100)}%` : "grade"}
				</span>
			</div>
			<div className="ve-look-browser__grid">
				{lookPresets.map((look) => (
					...
					>
						<span
							className="ve-look-browser__thumb"
							data-look-id={look.id}
							style={{ background: thumbnails?.[look.id] ?? look.preview }}
						/>
			...
			<label className="ve-slider-field">
				<span>
					{canAdjustIntensity ? "Look intensity" : "Look intensity unavailable"}
				</span>
				<input
					type="range"
					...
					onChange={(event) =>
						onIntensityChange(Number(event.currentTarget.value) / 100)
					}
				/>
				{canAdjustIntensity ? null : (
					<small>
						Select Cinema, Golden, Cool, or Mono to adjust intensity.
					</small>
				)}
			</label>
		</section>
