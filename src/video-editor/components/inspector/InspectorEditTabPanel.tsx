...
import {
	...
	sampleVideoFramePalette,
} from "../../color/framePalette";
...
const EffectEntry = ({
	...
	const attrs = useAttrs(["name", "kind"]) as {
		name?: unknown;
		kind?: unknown;
	};
	...
	return (
		<li>
			...
			<IconButton
				...
				variant="ghost"
				disabled={!effectId}
				onClick={() => (effectId ? onRemove(effectId) : undefined)}
			/>
		</li>
	);
};
	...
	<div className="ve-field-grid">
		...
		<label>
			...
			<input
				...
				onChange={(event) => onChange({ y: Number(event.currentTarget.value) })}
			/>
		</label>
		<label>
			<span>Scale</span>
			<input
				...
				onChange={(event) =>
					onChange({ scale: Number(event.currentTarget.value) })
				}
			/>
		</label>
		<label>
			...
			<input
				...
				onChange={(event) =>
					onChange({ rotation: Number(event.currentTarget.value) })
				}
			/>
		</label>
	</div>
...
const TextEditorSection = ({
	...
	const [paletteStatus, setPaletteStatus] =
		useState<FramePaletteStatus>("idle");
	...
	const applyFramePalette = (): void => {
		...
		const framePalette = topmostVideo
			? sampleVideoFramePalette(topmostVideo)
			: null;
...
export const InspectorEditTabPanel = ({
	mediaElementRegistry,
}: {
	mediaElementRegistry?: PreviewMediaElementRegistry;
}) => {
			...
			<InspectorSection title="Opacity" icon={Gauge}>
				<label className="ve-slider-field">
					...
					<input
						...
						onChange={(event) =>
							clipDispatch("updateOpacity", {
								opacityPercent: Number(event.currentTarget.value),
							})
						}
					/>
				</label>
			...
			<InspectorSection title="Fade" icon={Gauge} ariaLabel="Fade controls">
				...
				<div className="ve-button-grid">
					<Button
						type="button"
						variant="secondary"
						onClick={() => clipDispatch("setFade", { edge: "in", delta: 0.5 })}
					>
						Fade in +0.5s
					</Button>
					<Button
					...
					<Button
						type="button"
						variant="secondary"
						onClick={() => clipDispatch("setFade", { edge: "out", delta: 0.5 })}
					>
						Fade out +0.5s
					</Button>
					<Button
						...
						variant="secondary"
						onClick={() =>
							clipDispatch("setFade", { edge: "out", delta: -0.5 })
						}
						disabled={fadeOut <= 0}
					>
						Fade out -0.5s
			...
			<InspectorSection title="Trim" icon={Scissors} ariaLabel="Trim controls">
				<div className="ve-button-grid">
					<Button
						type="button"
						variant="secondary"
						onClick={() => clipDispatch("trim", { edge: "start", delta: 0.5 })}
					>
						Start +0.5s
					</Button>
					<Button
					...
					<Button
						type="button"
						variant="secondary"
						onClick={() => clipDispatch("trim", { edge: "end", delta: -0.5 })}
			...
			>
				<div className="ve-button-grid">
					<IconButton
						...
						label="Blur"
						variant="secondary"
						onClick={() => clipDispatch("addEffect", { kind: "blur" })}
					>
						Blur
					</IconButton>
					<IconButton
						...
						label="Sharpen"
						variant="secondary"
						onClick={() => clipDispatch("addEffect", { kind: "sharpen" })}
					>
						Sharpen
					</IconButton>
					<IconButton
						...
						label="Tint"
						variant="secondary"
						onClick={() => clipDispatch("addEffect", { kind: "tint" })}
					>
						Tint
					</IconButton>
				</div>
				<div className="ve-effects-toolbar">
					...
					{effectScopes.length > 0 ? (
						<div className="ve-effects-menu">
							...
							{isEffectsMenuOpen ? (
								<ul
									className="ve-effects-menu__list"
									aria-label="Active effects"
								>
									{effectScopes.map((effectScope) => (
										<ScopeContext.Provider
											key={effectScope._nodeId}
											value={effectScope}
										>
											<EffectEntry
												onRemove={(effectId) => {
