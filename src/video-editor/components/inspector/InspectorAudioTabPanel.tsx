...
import { ROOT_SCOPE, useEditorActions, useEditorAttrs, useEditorOne } from '../../render-sync'
		...
		<div className="ve-inspector-tab-panel" role="tabpanel" aria-label="Audio inspector">
			<InspectorSection title="Clip audio" icon={Volume2}>
				<label className="ve-slider-field">
					<span>Gain</span>
					<input type="range" aria-label="Gain" min="0" max="150" value={Math.round((audio?.gain ?? 1) * 100)} disabled={!isAudioClip} onChange={(event) => clipDispatch('setAudio', { gain: Number(event.currentTarget.value) / 100 })} />
				</label>
