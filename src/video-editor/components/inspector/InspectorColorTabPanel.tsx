...
import {
	buildLookColorCorrectionParams,
	getLookPreset,
	lookPresets,
} from "../../color/looks";
import type {
	ColorCorrectionAttrs,
	EffectAttrs,
...
const ColorCorrectionControls = ({
	...
	const colorCorrectionAttrs = useAttrs([
		"enabled",
		"params",
	]) as unknown as EffectAttrs;
	const effectId = effectScope?._nodeId ?? null;
	const colorParams = (colorCorrectionAttrs.params ??
		{}) as Partial<ColorCorrectionAttrs>;
	const isColorCorrectionEnabled = colorCorrectionAttrs.enabled !== false;
	const activeLookId =
		typeof (colorParams as Record<string, unknown>).lookId === "string"
	...
	const updateColorParams = (
		params: Partial<Record<ColorParamKey, number>> &
	...
	const lookIntensity = Number(
		(colorParams as Record<string, { value?: unknown }>).lookIntensity?.value ??
			1,
	);
	const applyLook = (
		nextLookId: string,
		nextIntensity = nextLookId === activeLookId ? lookIntensity : 1,
	): void => {
		activeLookIdRef.current = nextLookId;
		updateColorParams(
			buildLookColorCorrectionParams(nextLookId, nextIntensity),
		);
	};
	...
	useEffect(() => {
		...
		worker.onmessage = (
			...
			if (
				!isCancelled &&
				event.data.type === "look-thumbnails-rendered" &&
				event.data.thumbnails
			) {
		...
		worker.postMessage(
			{
				...
				looks: lookPresets.map((look) => {
					const {
						lookId: _lookId,
						lookIntensity: _lookIntensity,
						...params
					} = buildLookColorCorrectionParams(look.id, lookIntensity);
	...
	return (
		<>
			<div className="ve-color-grade-actions">
				...
				<Button type="button" variant="outline" onClick={resetGrade}>
					Reset grade
				</Button>
				<Button
					type="button"
			...
			<label className="ve-slider-field">
				...
				<input
					...
					min="-100"
					max="100"
					value={Math.round(getParamValue("exposure", 0) * 100)}
					onChange={(event) =>
						updateParam("exposure", Number(event.currentTarget.value) / 100)
					}
				/>
			</label>
			<label className="ve-slider-field">
				...
				<input
					...
					min="0"
					max="200"
					value={Math.round(getParamValue("contrast", 1) * 100)}
					onChange={(event) =>
						updateParam("contrast", Number(event.currentTarget.value) / 100)
					}
				/>
			</label>
			<label className="ve-slider-field">
				...
				<input
					...
					min="0"
					max="250"
					value={Math.round(getParamValue("saturation", 1) * 100)}
					onChange={(event) =>
						updateParam("saturation", Number(event.currentTarget.value) / 100)
					}
				/>
			</label>
			<label className="ve-slider-field">
				...
				<input
					...
					min="-100"
					max="100"
					value={Math.round(getParamValue("temperature", 0) * 100)}
					onChange={(event) =>
						updateParam("temperature", Number(event.currentTarget.value) / 100)
					}
				/>
			</label>
			<small>
				{isColorCorrectionEnabled ? "Grade active" : "Grade bypassed"} ·
				Exposure {getParamValue("exposure", 0).toFixed(2)} · Contrast{" "}
				{getParamValue("contrast", 1).toFixed(2)} · Saturation{" "}
				{getParamValue("saturation", 1).toFixed(2)}
			</small>
		</>
	);
};
...
export const InspectorColorTabPanel = ({
	...
	const [colorCorrectionEffectIds, setColorCorrectionEffectIds] = useState<
		ReadonlySet<string>
	>(() => new Set());
	const color = String(clipAttrs.color ?? "#2563eb");
	const hasColorCorrectionEffect = colorCorrectionEffectIds.size > 0;
	const handleColorCorrectionPresence = useCallback(
		(effectNodeId: string, present: boolean) => {
	...
	return (
		...
		>
			<InspectorSection title="Label color" icon={Palette}>
				<label className="ve-color-field">
					...
					<input
						...
						value={color}
						onChange={(event) =>
							clipDispatch("color", { color: event.currentTarget.value })
						}
					/>
				</label>
				<div className="ve-swatch-grid">
					{[
						...
						<button
							...
							onClick={() => clipDispatch("color", { color: swatch })}
						/>
					))}
				</div>
			</InspectorSection>
			<InspectorSection
				...
				ariaLabel="Primary color correction"
			>
