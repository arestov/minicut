...
import {
	FramePaletteAction,
	type FramePaletteStatus,
} from "./FramePaletteAction";
import {
	cssColorToHex,
	OklchColorField,
	transparentBackgroundFallback,
} from "./OklchColorField";
...
export const TextAppearancePanel = ({
	...
	const textBackgroundColor = cssColorToHex(
		...
		<>
			...
			<div className="ve-field-grid">
				<label>
					...
					<input
						...
						onChange={(event) =>
							onStyleChange({ fontSize: Number(event.currentTarget.value) })
						}
					/>
				</label>
				<label>
					...
					<input
						...
						onChange={(event) =>
							onStyleChange({ fontWeight: Number(event.currentTarget.value) })
						}
					/>
				</label>
				<label>
				...
				<label>
					...
					<input
						...
						onChange={(event) =>
							onStyleChange({ backgroundColor: event.currentTarget.value })
						}
					/>
				</label>
				<label>
					...
					<select
						...
						value={text.style.align}
						onChange={(event) =>
							onStyleChange({
								align: event.currentTarget.value as TextAttrs["style"]["align"],
							})
						}
					>
			...
			>
				...
				<Button
					...
					onClick={() =>
						onStyleChange({
							color: suggestReadableTextColor(textColor, textBackgroundColor),
						})
					}
				>
					Fix contrast
