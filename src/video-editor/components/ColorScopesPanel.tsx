...
import {
	drawScopeDensityCanvas,
	drawVectorscopePoints,
	parseScopeColor,
	type ScopeRgbColor,
} from "../render/colorScopeCanvas";
import {
	createPreviewScopeData,
	type PreviewScopeData,
	type RgbaSampleFrame,
	type ScopeDensityFrame,
} from "../render/colorScopes";
...
const getScopeSamplingStrategy = (): ScopeSamplingStrategy =>
	"displayed-preview-frame";
...
const imageSampleFrameCache = new Map<
	string,
	Promise<RgbaSampleFrame | null>
>();
...
const tryReadPreviewVideoSampleFrame = (
	...
	if (
		getScopeSamplingStrategy() === "state-frame" &&
		stateDrift > stateFramePreviewToleranceSeconds
	) {
		return null;
	}
...
const loadImageSampleFrame = (url: string): Promise<RgbaSampleFrame | null> =>
	new Promise((resolve) => {
...
const getCachedImageSampleFrame = (
	url: string,
): Promise<RgbaSampleFrame | null> => {
...
class VideoScopeSampler {
	...
	async sample(localTime: number): Promise<RgbaSampleFrame | null> {
		...
		const targetTime =
			duration > 0
				? Math.min(Math.max(0, localTime), Math.max(0, duration - 0.05))
				: Math.max(0, localTime);
		if (
			Math.abs(this.video.currentTime - targetTime) <= 0.025 &&
			this.video.readyState >= this.video.HAVE_CURRENT_DATA
		) {
...
const usePreviewScopeSamples = (
	...
	useEffect(() => {
		...
		const activeVideoSamplerKeys = new Set(
			clips
		...
		for (const clip of clips) {
			if (
				!clip.resourceUrl ||
				clip.resourceKind === "text" ||
				clip.resourceKind === "audio"
			) {
				setSamples((current) =>
					current[clip.id] === undefined
						? current
						: { ...current, [clip.id]: undefined },
				);
				continue;
			...
			const previewVideoSample =
				clip.resourceKind === "video"
					? tryReadPreviewVideoSampleFrame(
							clip,
			...
			if (previewVideoSample) {
				...
				setSamples((current) =>
					current[clip.id] === previewVideoSample
						? current
						: { ...current, [clip.id]: previewVideoSample },
				);
				continue;
			...
			const sampleSource: ScopeSampleSource =
				clip.resourceKind === "image" ? "image-cache" : "video-sampler";
			const samplePromise =
				clip.resourceKind === "image"
...
const WaveformScopeSection = ({ scopes }: { scopes: PreviewScopeData }) => (
	<ScopeDensityCanvas
		frame={scopes.waveform}
		tint={waveformTintColor}
		label="Waveform luma density"
	/>
);
...
const RgbParadeScopeSection = ({ scopes }: { scopes: PreviewScopeData }) => (
	<div className="ve-scopes__parade">
		<ScopeDensityCanvas
			frame={{
				width: scopes.rgbParade.width,
				height: scopes.rgbParade.height,
				cells: scopes.rgbParade.red,
			}}
			tint={redParadeTintColor}
			label="Red parade density"
		/>
		<ScopeDensityCanvas
			frame={{
				width: scopes.rgbParade.width,
				height: scopes.rgbParade.height,
				cells: scopes.rgbParade.green,
			}}
			tint={greenParadeTintColor}
			label="Green parade density"
		/>
		<ScopeDensityCanvas
			frame={{
				width: scopes.rgbParade.width,
				height: scopes.rgbParade.height,
				cells: scopes.rgbParade.blue,
			}}
			tint={blueParadeTintColor}
			label="Blue parade density"
		/>
	</div>
);
...
const VectorscopeSection = ({ scopes }: { scopes: PreviewScopeData }) => (
	<div className="ve-scopes__vectors" aria-label="Vectorscope points">
		<ScopeDensityCanvas
			...
			label="Vectorscope chroma density"
...
export const ColorScopesPanel = ({
	...
	const sampleFrames = usePreviewScopeSamples(
		resolvedClips,
		frame.cursor,
		mediaElementRegistry,
	);
	...
	return (
		<div className="ve-scopes" aria-label="Color scopes">
			<div className="ve-scopes__header">
				...
				<div className="ve-scopes__tabs" role="tablist" aria-label="Scope mode">
					<button
						...
						className={mode === "waveform" ? "is-active" : ""}
					...
					<button
						type="button"
						role="tab"
						aria-selected={mode === "rgb-parade"}
						className={mode === "rgb-parade" ? "is-active" : ""}
						onClick={() => onModeChange("rgb-parade")}
					>
						RGB Parade
					</button>
					<button
						...
						className={mode === "vectorscope" ? "is-active" : ""}
			...
			<div className="ve-scopes__plot" data-scope-mode={mode}>
				{isEmpty ? (
					<span className="ve-scopes__empty">No visual clip at cursor</span>
				) : null}
				{!isEmpty && mode === "waveform" ? (
					<WaveformScopeSection scopes={scopes} />
				) : null}
				{!isEmpty && mode === "rgb-parade" ? (
					<RgbParadeScopeSection scopes={scopes} />
				) : null}
