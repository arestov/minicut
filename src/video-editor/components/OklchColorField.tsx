...
export const OklchColorField = ({
	...
	return (
		<details
			className="ve-oklch-controls"
			aria-label={`${label} OKLCH controls`}
			open={defaultOpen}
		>
			...
			<label className="ve-slider-field" htmlFor={`${fieldId}-lightness`}>
				...
				<input
					...
					onChange={(event) =>
						updateOklch({ l: Number(event.currentTarget.value) / 100 })
					}
				/>
			</label>
			<label className="ve-slider-field" htmlFor={`${fieldId}-chroma`}>
				...
				<input
					...
					min="0"
					max="40"
					value={Math.round(oklch.c * 100)}
					onChange={(event) =>
						updateOklch({ c: Number(event.currentTarget.value) / 100 })
					}
				/>
			</label>
			<label className="ve-slider-field" htmlFor={`${fieldId}-hue`}>
				...
				<input
					...
					onChange={(event) =>
						updateOklch({ h: Number(event.currentTarget.value) })
					}
				/>
			</label>
		</details>
