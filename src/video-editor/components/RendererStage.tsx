...
import {
	type ColorProgram,
	mergeColorProgramCssFilters,
} from "../render/colorProgram";
import {
	compilePreviewLayerOperation,
	compilePreviewRenderPlan,
	getPreviewOperationValue,
	type PreviewLayerOperation,
} from "../render/previewRenderPlan";
...
const getCanvasClipSources = (
	structure: PreviewStructure,
	frame: PreviewFrame,
): PreviewCanvasClipSource[] => {
	const sourceClips =
		structure.clipSources.length > 0
...
const drawFallbackPreview = (
	...
	clips.forEach((clip, index) => {
		...
		context.fillText(
			`${clip.resourceKind}: ${clip.text?.content ?? clip.name}`,
			30,
			y + 14,
		);
	});
...
const getTextAlignItems = (
	align: NonNullable<RenderedClip["text"]>["style"]["align"],
...
const drawContainedVideoFrame = (
	...
	const width = Math.max(
		1,
		Math.round(rect.width || canvas.clientWidth || fallbackCanvasSize.width),
	);
	const height = Math.max(
		1,
		Math.round(rect.height || canvas.clientHeight || fallbackCanvasSize.height),
	);
...
const VisualClipLayer = ({
	...
	const filters = mergeColorProgramCssFilters(
		getPreviewOperationValue<ColorProgram[]>(
			layerOperation.operations,
			"effect",
			[],
		),
	);
	const opacity = getPreviewOperationValue<number>(
		layerOperation.operations,
		"opacity",
		clip.opacity,
	);
	const transform = getPreviewOperationValue<RenderedClip["transform"]>(
		layerOperation.operations,
		"transform",
		clip.transform,
	);
	const text = getPreviewOperationValue<RenderedClip["text"]>(
		layerOperation.operations,
		"text",
		clip.text,
	);
...
const AudioClipElement = ({
	...
	const audio = getPreviewOperationValue<RenderedClip["audio"]>(
		layerOperation.operations,
		"audio",
		clip.audio,
	);
...
export const RendererStage = ({
	...
	const previewRenderPlan = useMemo(
		() => compilePreviewRenderPlan(frame),
		[frame],
	);
	...
	const visualLayers = useMemo<PreviewLayerViewModel[]>(
		() =>
			frame.visualRenderedClips.map((clip, layerIndex) => {
				const operation =
					previewLayerByClipId.get(clip.id) ??
					compilePreviewLayerOperation(clip);
				return {
					clip,
					layerIndex,
					operation,
			...
			<div className="ve-renderer__safe-area">
				...
				{isSplitCompare ? (
					...
					>
						...
						<span className="ve-renderer__compare-label ve-renderer__compare-label--before">
							Before
						</span>
