...
const TimelineBody = ({
	...
	<div className="ve-timeline__body">
		<div
			...
			style={{ "--ve-track-count": trackItems.length } as CSSProperties}
		>
...
const ResolvedProjectTimeline = ({
	...
	timelineZoom,
	updateCursorFromPointer,
}: Omit<TimelineBodyProps, "trackItems"> & {
...
export const TimelineView = () => {
	...
	const canStartPlayheadDrag = (
		...
		return !(
			target?.closest(".ve-clip") || target?.closest("button, input, label")
		);
	};
	...
	const renderHeader = (trackCount: number) => (
		<TimelineHeader
			...
			onNudgeSelected={(delta) =>
				sessionDispatch("nudgeSelectedClip", { delta })
			}
