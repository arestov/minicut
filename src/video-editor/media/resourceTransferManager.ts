...
export interface ResourceTransferManager {
	...
	syncResources(
		resources: Array<{ resourceId: string; attrs: ResourceAttrs }>,
	): void;
	registerLocalResource(
		resourceId: string,
		file: File | Blob,
		snapshot: Omit<ResourceSnapshot, "resourceId"> & { objectUrl: string },
	): void;
	attachClientTransport(transport: P2PRawTransportLike): void;
	attachServerTransport(
		remotePeerId: string,
		transport: P2PRawTransportLike,
	): void;
...
const normalizeSourceKind = (value: unknown): ResourceSourceKind =>
	value === "p2p" ? "p2p" : "local";
...
const toSnapshot = (
	...
	ownerPeerId:
		typeof attrs.source?.ownerPeerId === "string"
			? attrs.source.ownerPeerId
			: null,
...
export const createResourceTransferManager = (
	...
	const pendingOwnerRequests = new Map<
		string,
		Array<{ peerKey: string; request: RequestMessage }>
	>();
	let destroyed = false;
	...
	const getTransport = (peerKey: string): AttachedTransport | null =>
		transports.get(peerKey) ?? null;
	...
	const recomputeRemoteStatus = (
		state: RemoteResourceState,
		resourceId: string,
	): void => {
		if (
			typeof state.size === "number" &&
			state.loadedBytes >= state.size &&
			state.pendingRequestIds.size === 0
		) {
			const elapsedSinceLastChunk = Math.max(
				0,
				Date.now() - state.lastTouchedAt,
			);
	...
	const queuePendingOwnerRequest = (
		resourceId: string,
		peerKey: string,
	...
	const flushPendingOwnerRequests = (
		resourceId: string,
	...
	const enforceCacheCap = (exemptResourceId?: string): void => {
		let totalBytes = Array.from(remoteStates.values()).reduce(
			(sum, state) => sum + state.loadedBytes,
			0,
		);
		...
		const candidates = Array.from(remoteStates.values())
			.filter(
				(state) =>
					state.resourceId !== exemptResourceId && state.loadedBytes > 0,
			)
	...
	const buildRemoteBlob = (
		...
		for (const index of getChunkIndexesForRange(
			[0, targetEnd],
			state.chunkSize,
		)) {
	...
	const buildSparsePreviewBlob = (state: RemoteResourceState): Blob | null => {
		...
		if (
			typeof totalSize === "number" &&
			Number.isFinite(totalSize) &&
			totalSize > MAX_SPARSE_PREVIEW_BLOB_BYTES
		) {
			return buildRemoteBlob(state, false);
		}

		const headRange = getHeadPreviewRange(
			state.size,
			state.chunkSize,
			headBytes,
		);
		...
		const preserveOriginalOffsets = selectedRanges.some(
			([start], index) => index > 0 && start > selectedRanges[index - 1][1],
		);
		const parts: BlobPart[] = [];
		let cursor = 0;
	...
	const updateTransferView = (resourceId: string): void => {
		...
		const totalBytes =
			state.size ?? (state.status === "ready" ? state.loadedBytes : 0);
		const previewUrl =
			state.playbackUrl ||
			state.previewUrl ||
			(isRealMediaUrl(state.fallbackUrl) ? state.fallbackUrl : "");
		transfers$.setItem(resourceId, {
			resourceId,
			name: state.name,
			kind: state.kind,
			...
			mode:
				state.ownerPeerId && state.ownerPeerId !== options.getPeerId()
					? "streaming"
					: "mirrored",
		});
	};
	...
	const rebuildPreviewUrls = (resourceId: string): void => {
		...
		const partialBlob =
			completeBlob ??
			buildSparsePreviewBlob(state) ??
			buildRemoteBlob(state, false);
		...
		if (completeBlob) {
			...
			state.status =
				typeof totalBytes === "number" && state.loadedBytes >= totalBytes
					? "ready"
					: "partial";
			updateTransferView(resourceId);
			return;
	...
	const isCurrentTransportGeneration = (
		...
		getTransport(peerKey)?.generation === generation;
	...
	const getRequestPeerKey = (snapshot: ResourceSnapshot): string | null => {
		if (options.getRole() === "server") {
			const ownerPeerId =
				snapshot.ownerPeerId && snapshot.ownerPeerId !== options.getPeerId()
			...
			const connectedPeerKeys = Array.from(transports.keys()).filter(
				(peerKey) => peerKey !== SERVER_TRANSPORT_KEY,
			);
	...
	const requestRanges = (
		...
		const missingRanges = subtractByteRanges(normalizedRanges, [
			...state.loadedRanges,
			...state.requestedRanges,
		]);
		...
		state.requestedRanges = mergeByteRanges([
			...state.requestedRanges,
			...missingRanges,
		]);
		state.requestedRangesLog = mergeByteRanges([
			...state.requestedRangesLog,
			...missingRanges,
		]);
	...
	const planNextRequest = (resourceId: string): void => {
		...
		const headMissing = headRange
			? subtractByteRanges([headRange], state.loadedRanges)
			: [];
		if (headMissing.length > 0) {
			requestRanges(
				resourceId,
				headMissing,
				options.getRole() === "server" ? "replication" : "head",
			);
			return;
		}

		if (state.tailRequested) {
			const tailRange = getTailFallbackRange(
				state.size,
				state.chunkSize,
				tailBytes,
			);
			const tailMissing = tailRange
				? subtractByteRanges([tailRange], state.loadedRanges)
				: [];
		...
		if (state.lastWindowRange) {
			const windowMissing = subtractByteRanges(
				[state.lastWindowRange],
				state.loadedRanges,
			);
		...
		requestRanges(
			resourceId,
			[sequentialRange],
			options.getRole() === "server" ? "replication" : "sequential",
		);
	};

	const ensureRemoteState = (
		snapshot: ResourceSnapshot,
	): RemoteResourceState => {
	...
	const applyChunkMeta = (
		...
		state.loadedRanges = mergeByteRanges([
			...state.loadedRanges,
			[meta.start, meta.end],
		]);
		state.requestedRanges = subtractByteRanges(state.requestedRanges, [
			[meta.start, meta.end],
		]);
		appendRequestEvent(state, {
			reason: meta.reason,
		...
		state.loadedBytes = Array.from(state.chunks.values()).reduce(
	...
	const handleRequest = (peerKey: string, message: RequestMessage): void => {
		...
		if (
			options.getRole() === "server" &&
			state.ownerPeerId &&
			state.ownerPeerId !== options.getPeerId()
		) {
		...
		syncResources(resources) {
			syncSnapshots(
				resources.map((resource) =>
					toSnapshot(resource.resourceId, resource.attrs, defaultChunkSize),
				),
			);
		},
