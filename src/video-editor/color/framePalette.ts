...
export const createPaletteFromRgbSamples = (
	samples: RgbColor[],
): FramePaletteSuggestion | null => {
	if (samples.length === 0) {
		return null;
	}

	const average = samples.reduce<RgbColor>(
		(sum, color) => ({
			r: sum.r + color.r,
			g: sum.g + color.g,
			b: sum.b + color.b,
	...
	const backgroundColor = oklchToHex({
		...baseOklch,
		l: clamp(
			baseOklch.l < 0.5 ? baseOklch.l * 0.62 : baseOklch.l * 0.52,
			0.04,
			0.72,
		),
	...
	const accentColor = oklchToHex({
		l: clamp(
			baseOklch.l < 0.5 ? baseOklch.l + 0.28 : baseOklch.l - 0.18,
			0.22,
			0.86,
		),
	...
	return {
		...
		accentColor,
		contrastRatio: Number(
			getContrastRatio(textColor, backgroundColor).toFixed(2),
		),
	};
};

export const createPaletteFromHex = (
	value: string,
): FramePaletteSuggestion | null => {
...
export const collectCanvasRgbSamples = (
	imageData: ImageData,
	stride = 4,
): RgbColor[] => {
...
export const readVideoFrameImageData = (
...
export const sampleVideoFramePalette = (
	video: HTMLVideoElement,
): FramePaletteSuggestion | null => {
	const imageData = readVideoFrameImageData(video, 32);
	return imageData
		? createPaletteFromRgbSamples(collectCanvasRgbSamples(imageData, 3))
		: null;
};
...
export const normalizePaletteHex = (value: string, fallback: string): string =>
	hexToOklch(value)
		? rgbToHex(
				parseHexColor(value) ?? parseHexColor(fallback) ?? { r: 0, g: 0, b: 0 },
			)
		: fallback;
