...
export const rgbToOklch = (rgb: RgbColor): OklchValue => {
	...
	const hue = chroma < 0.00001 ? 0 : (Math.atan2(okB, okA) * 180 / Math.PI + 360) % 360

	return { l: round(okL), c: round(chroma), h: round(hue, 2) }
}

const oklchToRgbRaw = ({ l, c, h }: OklchValue): { r: number; g: number; b: number } => {
...
export const fitOklchToSrgb = (value: OklchValue): OklchValue => {
	let fitted = { l: clamp(value.l, 0, 1), c: Math.max(0, value.c), h: ((value.h % 360) + 360) % 360 }
...
export const oklchToRgb = (value: OklchValue): RgbColor => {
	...
	return { r: linearToSrgb(raw.r), g: linearToSrgb(raw.g), b: linearToSrgb(raw.b) }
}

export const oklchToHex = (value: OklchValue): string => rgbToHex(oklchToRgb(value))
...
export const getContrastResult = (foreground: string, background: string): ContrastResult => {
...
export const suggestReadableTextColor = (foreground: string, background: string): string => {
