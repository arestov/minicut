...
export const buildLookColorCorrectionParams = (
	...
	return {
		...
		exposure:
			neutralLookParams.exposure +
			(preset.params.exposure - neutralLookParams.exposure) * clampedIntensity,
		contrast:
			neutralLookParams.contrast +
			(preset.params.contrast - neutralLookParams.contrast) * clampedIntensity,
		saturation:
			neutralLookParams.saturation +
			(preset.params.saturation - neutralLookParams.saturation) *
				clampedIntensity,
		temperature:
			neutralLookParams.temperature +
			(preset.params.temperature - neutralLookParams.temperature) *
				clampedIntensity,
		hue:
			neutralLookParams.hue +
			(preset.params.hue - neutralLookParams.hue) * clampedIntensity,
		gamma:
			neutralLookParams.gamma +
			(preset.params.gamma - neutralLookParams.gamma) * clampedIntensity,
	};
};
