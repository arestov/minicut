...
import { createActionContractHarness, dispatchAndSettle } from '../dkt/models/action-contract-test-harness'

const findProjectExportRequest = (
	exportRequests: unknown[],
	id: string,
): ExportRequestState | null => {
	const match = exportRequests.find((entry) =>
...
describe('render export task executor contract', () => {
	it('sends renderer request with node-based resourceId and resolved resourceUrl for project export', async () => {
		...
		const exportResource = resources.find((resource) => harness.ctx.getAttr(resource, 'name') === 'Export Source')
		...
		const exportClip = clips.find((clip) => {
			const renderData = harness.ctx.getAttr(clip, 'clipRenderData') as { resourceId?: unknown } | null
		...
		await dispatchAndSettle(harness.ctx, harness.sessionRoot, 'requestProjectExport', {
			id: requestId,
		...
		const planClipBeforeResolve = request.plan.clipSources.find((clipSource) =>
			clipSource.id === exportClipId && clipSource.resourceId === exportResourceId)
		expect(planClipBeforeResolve).toBeTruthy()
		expect(planClipBeforeResolve?.resourceUrl).toBe('https://example.invalid/export-source.webm')

		const resolvedUrl = 'blob:resolved-export-source'
		const renderer = {
			render: vi.fn(async (renderRequest: ExportRenderRequest): Promise<ExportRenderResult> => ({
				id: requestId,
		...
		const task = taskPort.dispatchTask('$fx_renderExport', { data: request }, {
		...
		const rendererRequest = renderer.render.mock.calls[0]?.[0] as ExportRenderRequest
		const planClipSentToRenderer = rendererRequest.plan.clipSources.find((clipSource) =>
			clipSource.id === exportClipId && clipSource.resourceId === exportResourceId)
		...
		expect(dktDispatches.some((entry) => entry.actionName === 'setExportProgress')).toBe(true)
		expect(dktDispatches.some((entry) =>
			entry.actionName === 'consumeExportRequest'
			&& (entry.payload as { id?: unknown } | null)?.id === requestId)).toBe(true)
	})
	...
	it('fails fast before renderer when media clip has empty resolved resourceUrl', async () => {
		...
		const mediaClip = request.plan.clipSources.find((clipSource) =>
			clipSource.resourceKind === 'video' || clipSource.resourceKind === 'audio' || clipSource.resourceKind === 'image')
		...
		const brokenRequest: ExportRequestState = {
			...
			plan: {
				...
				clipSources: request.plan.clipSources.map((clipSource) =>
					clipSource.id === mediaClip.id ? { ...clipSource, resourceUrl: '' } : clipSource),
			},
		}

		const renderer = {
			render: vi.fn(async (): Promise<ExportRenderResult> => ({
				id: requestId,
				fileName: 'export.webm',
				mimeType: 'video/webm',
				blob: new Blob(['export-bytes'], { type: 'video/webm' }),
		...
		const taskPort = createRuntimeTaskFacade()
		const task = taskPort.dispatchTask('$fx_renderExport', { data: brokenRequest }, {
			queuePolicy: 'replace-last',
			intentKey: '$fx_renderExport:project',
		})
		...
		const errorProgressDispatch = dktDispatches.find((entry) =>
			entry.actionName === 'setExportProgress'
			&& (entry.payload as { stage?: unknown } | null)?.stage === 'error')
		expect(errorProgressDispatch).toBeTruthy()
		expect(String((errorProgressDispatch?.payload as { error?: unknown } | null)?.error ?? '')).toContain('missing resourceUrl')
		expect(dktDispatches.some((entry) =>
			entry.actionName === 'consumeExportRequest'
			&& (entry.payload as { id?: unknown } | null)?.id === requestId)).toBe(true)
