export type RuntimeTaskQueuePolicy =
	| "replace-last"
	| "queue-all"
	| "keep-first";
...
export const createRuntimeTaskFacade = () => {
	...
	const dispatchTask = (
		...
		if (!fxName.startsWith("$fx_")) {
			throw new Error(
				`Runtime task name must start with $fx_, received ${fxName}`,
			);
		}
		if (payload.data !== undefined && !isSerializable(payload.data)) {
			throw new Error(
				`Runtime task payload.data must be serializable for ${fxName}`,
			);
		}
		...
		const runtimeHandleId =
			payload.runtimeRef === undefined
				? undefined
				: putRuntimeRef(payload.runtimeRef);
	...
	const completeTask = (
		task: Pick<RuntimeTaskDescriptor, "taskId" | "intentKey">,
	): void => {
		...
		if (
			taskHistory.has(task.taskId) &&
			taskStatus.get(task.taskId) === "active"
		) {
