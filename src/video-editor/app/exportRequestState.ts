...
const parsePlan = (value: unknown): ExportPlan | null => {
	...
	if (
		...
		!Array.isArray(plan.clipSources)
	) {
		return null;
	}
	return {
		...
		fps:
			typeof plan.fps === "number" && Number.isFinite(plan.fps) ? plan.fps : 30,
		width:
			typeof plan.width === "number" && Number.isFinite(plan.width)
				? plan.width
				: 1920,
		height:
			typeof plan.height === "number" && Number.isFinite(plan.height)
				? plan.height
				: 1080,
		duration:
			typeof plan.duration === "number" && Number.isFinite(plan.duration)
				? plan.duration
				: 0,
...
export const parseExportRequest = (
	value: unknown,
	...
	const format: ExportRequestFormat =
		request.format === "video-webm" ? "video-webm" : "video-webm";
	return {
		id,
		range,
		format,
