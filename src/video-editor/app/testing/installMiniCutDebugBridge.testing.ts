...
const getActiveProjectScopeTesting = (harness: VideoEditorHarness): ReactSyncScopeHandle | null => {
...
export const installMiniCutDebugBridgeTesting = (harness: VideoEditorHarness): (() => void) => {
	...
	const debug: MiniCutDebugBridge = {
		...
		dumpGraphSummary: () => summarizeGraph(harness.pageRuntime?.debugDumpGraph?.() ?? null),
		...
		getResourceTransfers: () => Object.values(harness.resourceTransfers$.get()).map((transfer) => ({
		...
		dumpWorkerState: () => harness.pageRuntime?.requestDebugDump?.() ?? Promise.resolve(null),
		getRole: () => {
