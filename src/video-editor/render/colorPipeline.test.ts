...
import {
	...
	mergeEffectFilters,
	toEffectRenderInstruction,
} from "./colorPipeline";

describe("color pipeline", () => {
	it("normalizes effect attrs into render instructions", () => {
		const attrs: EffectAttrs = {
			name: "Primary",
			kind: "color-correction",
			enabled: true,
			params: { exposure: { value: 0.25 }, saturation: { value: 1.4 } },
		};
		...
		expect(instruction.params).toMatchObject({
			exposure: { value: 0.25 },
	...
	it("compiles deterministic css filters for color correction params", () => {
		...
		expect(filter).toBe(
			"brightness(1.25) contrast(1.2) saturate(1.5) hue-rotate(12deg)",
		);
	});
