...
export const compileEffectColorProgram = (
	effect: EffectRenderInstruction,
): ColorProgram => {
	if (!effect.enabled) {
	...
	if (effect.kind === "blur") {
		return {
			...
			enabled: true,
			operations: [{ type: "blur", value: Math.round(amount * 24) / 4 }],
		};
	}
	...
	if (effect.kind === "color-correction") {
		...
		const saturationValue = clamp(
			saturation + Math.max(0, temperature) * 0.08,
			0,
			4,
		);
		...
		return {
			...
			operations: [
				...
				...(sepiaValue > 0
					? [{ type: "sepia" as const, value: round(sepiaValue) }]
					: []),
...
export const colorProgramOperationToCss = (
	operation: ColorProgramOperation,
): string => {
...
export const colorProgramToCssFilter = (program: ColorProgram): string =>
	program.enabled
		? program.operations
				.map(colorProgramOperationToCss)
				.filter(Boolean)
				.join(" ")
		: "";
