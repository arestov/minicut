...
const webCodecsAudioCandidates: Array<{
	encoderCodec: string;
	muxerCodec: "A_OPUS";
...
const clampAudioSample = (sample: number): number =>
	Math.max(-1, Math.min(1, sample));

const resolveStereoSample = (
	buffer: AudioBuffer,
	...
	const sampleAt = (channel: Float32Array): number =>
		channel[frameIndex] +
		(channel[nextIndex] - channel[frameIndex]) * interpolation;
...
export const mixWebCodecsAudioTrack = async (
	...
	const audioClips = getRangeClips(plan, resolvedRange).filter(
	...
	try {
		for (const clip of audioClips) {
			...
			const destinationStartFrame = Math.max(
				0,
				Math.floor((activeStart - exportStart) * sampleRate),
			);
			...
			const sourceOffsetSeconds =
				clip.trimStart + Math.max(0, exportStart - clip.start);
			...
			for (
				let frameOffset = 0;
				frameOffset < destinationFrameCount;
				frameOffset += 1
			) {
				const sourcePosition = sourceStartFrame + frameOffset * sourceFrameStep;
				if (
					!Number.isFinite(sourcePosition) ||
					sourcePosition >= decodedBuffer.length - 1
				) {
					break;
				}
				const [leftSample, rightSample] = resolveStereoSample(
					decodedBuffer,
					sourcePosition,
				);
		...
		for (let offset = 0; offset < totalFrames; offset += framesPerChunk) {
			...
			for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
				for (let channel = 0; channel < numberOfChannels; channel += 1) {
					interleaved[frameIndex * numberOfChannels + channel] =
						frames[channel][offset + frameIndex] ?? 0;
				}
			}
...
export const createAudioExportMixer = async (
	...
	for (const clip of audioClips) {
		...
		const stereoPanner =
			"createStereoPanner" in audioContext
		...
		async start() {
			...
			for (const clip of decodedClips) {
				...
				const activeDuration = Math.min(
					...
					Math.max(0, clip.buffer.duration - bufferOffset),
				);
				if (activeDuration <= 0) {
					continue;
				}
			...
			for (const clip of elementClips) {
				...
				elementTimers.push(
					globalThis.setTimeout(() => clip.element.pause(), stopMs),
				);
			}
		},
		async stop() {
