...
const createCells = (width: number, height: number): Float32Array =>
	new Float32Array(width * height);
const createZeroCells = (width: number, height: number): number[] =>
...
const compileFilterAdjustments = (
	filters: string[],
): CompiledFilterAdjustments => {
	let brightness = 1;
	let contrast = 1;
	let saturation = 1;
	let hueDegrees = 0;
	...
	for (let y = 0; y < height; y += 1) {
		for (let x = 0; x < width; x += 1) {
			...
			data[offset + 1] = Math.round(
				clamp01(baseGreen * gradient * vertical) * 255,
			);
			data[offset + 2] = Math.round(
				clamp01(baseBlue * gradient * vertical) * 255,
			);
			data[offset + 3] = 255;
		}
	}
...
const getClipSampleFrame = (
	clip: RenderedClip,
	sampleFrames: ScopeSampleFrames,
): { frame: RgbaSampleFrame; sampled: boolean } => {
	const sample =
		sampleFrames[clip.id] ??
		(clip.resourceId ? sampleFrames[clip.resourceId] : undefined);
	if (
		sample &&
		sample.width > 0 &&
		sample.height > 0 &&
		sample.data.length >= sample.width * sample.height * 4
	) {
...
const getVectorscopeCoordinates = (
	red: number,
	green: number,
	blue: number,
	...
	return {
		...
		intensity: Math.max(
			0.05,
			Math.min(1, Math.abs(chromaRed) + Math.abs(chromaBlue)),
		),
	};
};
...
export const createPreviewScopeData = (
	...
	const includeVectorscopePoints =
		options.includeVectorscopePoints ?? includeVectorscope;
	const clipScopeSources = buildClipScopeSources(clips, sampleFrames);
	const visualClips = clipScopeSources.map((source) => source.clip);
	const waveform = includeWaveform
		? createCells(waveformWidth, waveformHeight)
		: null;
	const redParade = includeRgbParade
		? createCells(paradeChannelWidth, paradeHeight)
		: null;
	const greenParade = includeRgbParade
		? createCells(paradeChannelWidth, paradeHeight)
		: null;
	const blueParade = includeRgbParade
		? createCells(paradeChannelWidth, paradeHeight)
		: null;
	const vectorscope = includeVectorscope
		? createCells(vectorscopeSize, vectorscopeSize)
		: null;
	...
	for (const source of clipScopeSources) {
		...
		const pointLimit =
			includeVectorscope && includeVectorscopePoints
				? maxVectorPoints * visualClips.length
				: 0;
		...
		for (let y = 0; y < height; y += 1) {
			for (let xIndex = 0; xIndex < width; xIndex += 1, pixelIndex += 1) {
				...
				const [red, green, blue] = applyFilterApproximation(
					rawRed,
					rawGreen,
					rawBlue,
					filterAdjustments,
				...
				if (includeVectorscope && vectorscope) {
					...
					if (
						pixelIndex % vectorStep === 0 &&
						vectorscopePoints.length < pointLimit
					) {
	...
	return {
		...
		waveform: {
			...
			cells: waveform
				? normalizeCells(waveform)
				: createZeroCells(waveformWidth, waveformHeight),
		},
		rgbParade: {
			type: "rgb-parade",
			...
			height: paradeHeight,
			red: redParade
				? normalizeCells(redParade)
				: createZeroCells(paradeChannelWidth, paradeHeight),
			green: greenParade
				? normalizeCells(greenParade)
				: createZeroCells(paradeChannelWidth, paradeHeight),
			blue: blueParade
				? normalizeCells(blueParade)
				: createZeroCells(paradeChannelWidth, paradeHeight),
		},
		vectorscope: {
			type: "vectorscope",
			...
			cells: vectorscope
				? normalizeCells(vectorscope)
				: createZeroCells(vectorscopeSize, vectorscopeSize),
