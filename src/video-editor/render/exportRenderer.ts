...
import {
	createAudioExportMixer,
	encodeMixedAudioTrack,
	getWebCodecsAudioConfig,
	mixWebCodecsAudioTrack,
} from "./audioExportMixer";
import {
...
import {
	createResourceCacheFromPlan,
	drawPreparedFrameOperations,
	prepareFrameOperations,
} from "./frameRenderer";
...
const webCodecsVideoCandidates: Array<{
	encoderCodec: string;
...
export const getFramePacingDelayMs = (
...
const renderWebCodecsVideoBlob = async ({
	...
	const audioClipCount = getRangeClips(plan, resolvedRange).filter(
		(clip) => clip.type === "ef-audio",
	).length;
	const audioConfig =
		audioClipCount > 0 ? await getWebCodecsAudioConfig(2, 128_000) : null;
	...
	try {
		for (let index = 0; index < frameCount; index += 1) {
			...
			encoder.encode(frame, {
				keyFrame: index % Math.max(1, Math.round(fps)) === 0,
			});
			frame.close();
...
const buildFileName = (rangeName: string, format: ExportFormat): string =>
	`${sanitizeFileNamePart(rangeName)}${format === "video-webm" ? ".webm" : ".minicut-export.json"}`;
...
export const createBrowserVideoExportRenderer = (
	...
	const fallbackToManifestOnUnsupported =
		options.fallbackToManifestOnUnsupported ?? true;
	...
	return {
		async render(request, onProgress) {
			...
			if (webCodecsAttempt.blob) {
				onProgress?.({ stage: "finalizing", progress: 1 });
				const fixedWebCodecsBlob = await fixWebmDuration(
					webCodecsAttempt.blob,
					duration * 1000,
					{ logger: false },
				);
				const diagnostics = createExportDiagnostics(
			...
			if (!isVideoExportSupported()) {
				if (fallbackToManifestOnUnsupported) {
					const result = await fallbackRenderer.render(
						{ ...request, format: "json-manifest" },
						onProgress,
					);
			...
			const audioMixer = await createAudioExportMixer(
				request.plan,
				resolvedRange,
				start,
				duration,
			...
			const stopPromise = new Promise<void>((resolve, reject) => {
				...
				recorder.addEventListener("error", () =>
					reject(new Error("Export recorder error")),
				);
			});
			...
			const blob = await fixWebmDuration(recordedBlob, duration * 1000, {
				logger: false,
			});
			const rangeName = getRangeName(request.plan, request.range);
			const result: ExportRenderResult = {
