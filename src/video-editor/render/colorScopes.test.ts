...
const countNonZero = (values: number[]): number =>
...
describe("color scopes", () => {
	it("returns empty density scopes without visual clips", () => {
		...
		expect(scopes.waveform).toMatchObject({
			type: "waveform",
			width: 128,
			height: 72,
		});
	...
	it("builds a diagonal waveform and neutral vectorscope from a grayscale ramp", () => {
		...
		expect(
			scopes.vectorscope.points.every(
				(point) => Math.abs(point.x) < 0.02 && Math.abs(point.y) < 0.02,
			),
		).toBe(true);
	});
	...
	it("separates RGB parade density for color-bar style samples", () => {
		...
		const colorBars = createFrame(
			64,
			16,
			(x) =>
				bars[Math.min(bars.length - 1, Math.floor((x / 64) * bars.length))],
		);
		...
		const vectorQuadrants = new Set(
			scopes.vectorscope.points.map(
				(point) => `${Math.sign(point.x)},${Math.sign(point.y)}`,
			),
		);

		expect(scopes.rgbParade).toMatchObject({
			type: "rgb-parade",
			width: 56,
			height: 72,
		});
	...
	it("computes only the requested scope section", () => {
		...
		const waveformOnly = createPreviewScopeData(
			[clip],
			{ [clip.id]: sampledFrame },
			{
		...
		const rgbOnly = createPreviewScopeData(
			[clip],
			{ [clip.id]: sampledFrame },
			{
	...
	it("changes sampled density when color correction filters change", () => {
		...
		const sampledFrame = createFrame(16, 16, (x, y) => [
			40 + x * 8,
			56 + y * 5,
			170,
		]);
		const base = createPreviewScopeData([clip], { [clip.id]: sampledFrame });
		const gradedClip = createClip({
			color: "#3344aa",
			filters: [
				"brightness(1.4) contrast(1.2) saturate(1.5) hue-rotate(24deg)",
			],
		});
		const graded = createPreviewScopeData([gradedClip], {
			[gradedClip.id]: sampledFrame,
		});

		expect(graded.waveform.cells).not.toEqual(base.waveform.cells);
		expect(graded.rgbParade.red).not.toEqual(base.rgbParade.red);
		expect(graded.vectorscope.points[0]).not.toEqual(
			base.vectorscope.points[0],
		);
	});
});
