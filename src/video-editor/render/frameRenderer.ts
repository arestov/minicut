...
const getOperationValue = <Value>(
	...
	(operations.find((operation) => operation.type === type)?.value as Value) ??
	fallback;
...
const toEffectInstruction = (value: unknown): EffectRenderInstruction => {
	...
	return {
		kind: String(value) as EffectRenderInstruction["kind"],
		name: String(value),
		enabled: true,
		amount: 1,
	};
...
const getEffectFilter = (effect: EffectRenderInstruction): string =>
	getEffectInstructionFilter(effect);
...
export const createResourceCache = (
	registryEntities: Record<
		string,
		{ type: string; attrs: unknown } | undefined
	>,
): Map<string, RenderableResource> => {
...
export const createResourceCacheFromPlan = (
	plan: ExportPlan,
): Map<string, RenderableResource> => {
	const cache = new Map<string, RenderableResource>();
...
const ensureRenderableResource = async (
	...
	if (kind === "image") {
		...
		resource.loadPromise = loadImageResource(resource.attrs.url).then(
			(image) => {
	...
	if (kind === "video") {
		...
		resource.loadPromise = loadVideoResource(resource.attrs.url).then(
			(video) => {
	...
	for (const operation of operations) {
		...
		if (operation.resourceKind === "image" && resource.image) {
			prepared.push({
				...
				sourceWidth:
					resource.image.naturalWidth || Number(resource.attrs.width) || width,
				sourceHeight:
					resource.image.naturalHeight ||
					Number(resource.attrs.height) ||
					height,
			});
			continue;
		}
		...
		if (operation.resourceKind === "video" && resource.video) {
			if (videoSyncMode === "seek-each-frame") {
				...
				if (
					shouldSeekRealtimeVideoFrame(
						resource.video.currentTime,
						operation.sourceTime,
						realtimeSeekTolerance,
					)
				) {
					await seekVideo(resource.video, operation.sourceTime);
				}
			...
			prepared.push({
				...
				sourceWidth:
					resource.video.videoWidth || Number(resource.attrs.width) || width,
				sourceHeight:
					resource.video.videoHeight || Number(resource.attrs.height) || height,
			});
			continue;
		}
...
export const drawPreparedFrameOperations = (
	...
	for (const {
		...
		sourceHeight,
		drawSource,
	} of preparedOperations) {
		if (operation.resourceKind === "audio") {
			continue;
		}

		const transform = getOperationValue<{
			x: number;
			y: number;
			scale: number;
			rotation: number;
		...
		const opacity = Math.max(
			...
			Math.min(
				1,
				Number(getOperationValue(operation.operations, "opacity", 1)) || 0,
			),
		);
		...
		const text = getOperationValue<TextAttrs | null>(
