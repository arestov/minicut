import type { PatchEnvelope, ProjectRegistry, RelValue } from './types'
	...
	for (const patch of envelope.patches) {
		switch (patch.c) {
			...
			case PATCH.ENTITY_SET: {
				...
				nextRegistry.entitiesById[patch.p.entity.id] = patch.p.entity
				break
			}
			...
			case PATCH.ATTRS_MERGE: {
				...
				const current = nextRegistry.entitiesById[patch.p.id]
				nextRegistry.entitiesById[patch.p.id] = {
					...current,
					attrs: {
						...current.attrs,
						...patch.p.attrs,
