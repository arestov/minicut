...
export const mergeByteRanges = (
	ranges: ResourceByteRange[],
): ResourceByteRange[] => {
	const sorted = ranges
		.map(
			([start, end]) =>
				[Math.floor(start), Math.floor(end)] as ResourceByteRange,
		)
		.filter(
			([start, end]) =>
...
const countReadyBytes = (chunks: Record<number, ResourceChunkMeta>): number =>
	Object.values(chunks).reduce(
		(total, chunk) =>
			total + (chunk.status === "ready" ? Math.max(0, chunk.size) : 0),
		0,
	);
...
export const getResourceDerived = (
	attrs: ResourceDerivedInput,
): ResourceDerived => {
