...
export const assertEntity = (project: ProjectGraph, entityId: string): Entity => {
...
const assertProjectGraphShape = (project: ProjectGraph): void => {
	const root = getProjectEntity(project)
	...
	assertEntityType(project, String(root.rels.activeTimeline), 'timeline')
}
...
export const validateCommand = (registry: ProjectRegistry, command: Command): void => {
	switch (command.c) {
		...
		case CMD.RESOURCE_IMPORT: {
			...
			assertProjectGraphShape(project)
		...
		case CMD.TIMELINE_ADD_CLIP: {
			const project = assertProject(registry, command.p.projectId)
			assertProjectGraphShape(project)
			assertEntityType(project, command.p.resourceId, 'resource')
			if (command.p.trackId) {
				const track = assertEntityType(project, command.p.trackId, 'track')
		...
		case CMD.TIMELINE_MOVE_CLIP: {
			const project = assertProject(registry, command.p.projectId)
			assertEntityType(project, command.p.clipId, 'clip')
		...
		case CMD.TIMELINE_SPLIT_CLIP: {
			const project = assertProject(registry, command.p.projectId)
			const clip = assertEntityType(project, command.p.clipId, 'clip')
