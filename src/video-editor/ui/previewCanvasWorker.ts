...
const interpolateLinear = (from: number, to: number, progress: number): number =>
	...
	for (let index = 0; index < keyframes.length - 1; index += 1) {
		...
		if (time >= from.time && time <= to.time) {
			return from.interpolation === 'hold' || from.time === to.time
				? from.value
				: interpolateLinear(from.value, to.value, (time - from.time) / (to.time - from.time))
		}
	}
...
const evaluateFadeOpacity = (
	...
	const fadeInMultiplier = clip.fadeIn > 0
		? Math.min(1, Math.max(0, localTime / clip.fadeIn))
		: 1
	const fadeOutStart = clip.duration - clip.fadeOut
	const fadeOutMultiplier = clip.fadeOut > 0 && localTime > fadeOutStart
...
self.onmessage = (event: MessageEvent<PreviewCanvasMessage>) => {
	...
	drawPreview(context, message.width, message.height, message.cursor, renderClipsAtCursor(message.cursor))
}
