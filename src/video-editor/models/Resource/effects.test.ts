...
import {
	createResourceRegisterLocalEffectPayload,
	createResourceTransferStatusEffectPayload,
	isResourceTransferStatusEffectData,
	RESOURCE_REGISTER_LOCAL_FX,
} from "./effects";
...
describe("Resource model effects", () => {
	it("keeps transfer status data serializable", () => {
		...
		expect(payload).toEqual({
			data: { resourceId: "resource:1", status: "partial", loadedBytes: 128 },
		});
	...
	it("keeps local files behind runtime refs", () => {
		...
		const task = tasks.dispatchTask(
			RESOURCE_REGISTER_LOCAL_FX,
			createResourceRegisterLocalEffectPayload(file, {
				resourceId: "resource:1",
				kind: "video",
				mime: "video/webm",
				duration: 4,
		...
		expect(task.payload.data).toMatchObject({
			resourceId: "resource:1",
			sourceKind: "local",
		});
		expect(tasks.consumeRuntimeRef(String(task.payload.runtimeHandleId))).toBe(
			file,
		);
	});
});
