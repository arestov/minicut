...
export type DktClipActionName =
	...
	| "removeEffect"
	| "reorderEffect";
export type DktTimelineClipActionName =
	| "moveBy"
	| "trim"
	| "resize"
	| "splitAt";

export type DktClipActionPatch = Partial<
	Pick<
		ClipAttrs,
		| "name"
		| "color"
	...
	>
>;
export type DktTimelineClipActionPatch = Partial<
	Pick<ClipAttrs, "start" | "in" | "duration">
>;
...
export const reduceClipUpdateOpacityAction = (
	payload: unknown,
): Pick<ClipAttrs, "opacity"> | null => {
	...
	const opacity =
		typeof opacityPercent === "number"
			? clipUpdateOpacityAction.fn(opacityPercent)
			: null;
...
export const reduceClipRenameAction = (
	payload: unknown,
): Pick<ClipAttrs, "name"> | null => {
...
export const reduceClipColorAction = (
	payload: unknown,
): Pick<ClipAttrs, "color"> | null => {
	const color = clipColorAction.fn(payload);
	return color === null ? null : { color };
};

export const reduceClipSetMediaKindAction = (
	payload: unknown,
): Pick<ClipAttrs, "mediaKind"> | null => {
	const mediaKind =
...
export const clipSetFadeAction = {
	fn(
		payload: unknown,
		clipAttrs: Pick<ClipAttrs, "fadeIn" | "fadeOut" | "duration">,
	): Pick<ClipAttrs, "fadeIn"> | Pick<ClipAttrs, "fadeOut"> | null {
		...
		return {
			[key]: clamp(roundToTenths(current + delta), 0, clipAttrs.duration),
		};
	},
};
...
export const clipSetTimelineAttrsAction = {
	...
	> | null {
		const value = payload as Partial<
			Pick<ClipAttrs, "start" | "in" | "duration" | "fadeIn" | "fadeOut">
		> | null;
...
export const clipSetTransformAction = {
	...
	): Pick<ClipAttrs, "transform"> {
		const partial = payload as Partial<
			Record<"x" | "y" | "scale" | "rotation", number>
		>;
		return {
			transform: {
...
export const toAnimatedScalar = (
	value: unknown,
	fallback: AnimatedScalar,
): AnimatedScalar => {
	if (
		value &&
		typeof value === "object" &&
...
export const getDktResizedClipAttrs = (
	...
	const nextStart = clamp(
		roundToTenths(attrs.start + delta),
		minStart,
		clipEnd - 0.5,
	);
	return {
...
export const reduceTimelineSplitAtAction = (
	...
	if (
		typeof time !== "number" ||
		time <= attrs.start ||
		time >= attrs.start + attrs.duration
	) {
		return null;
	}
...
export const normalizeEffectCreationAttrs = (payload: unknown) => {
	...
	const kindDerivedName = kindStr
		? kindStr.charAt(0).toUpperCase() + kindStr.slice(1)
		: null;
...
export const reorderEffectRefs = (
	effects: unknown[],
	effectId: unknown,
	toIndex: unknown,
): unknown[] | null => {
	...
	const currentIndex = effects.findIndex(
		(effect) => getNodeId(effect) === effectId,
	);
	if (currentIndex < 0) {
		return null;
	}
	const withoutEffect = effects.filter(
		(effect) => getNodeId(effect) !== effectId,
	);
	const nextIndex = Math.max(0, Math.min(toIndex, withoutEffect.length));
	return [
		...withoutEffect.slice(0, nextIndex),
		effects[currentIndex],
		...withoutEffect.slice(nextIndex),
	];
};

export const removeEffectRef = (
	effects: unknown[],
	effectId: unknown,
): unknown[] | null => {
	if (
		typeof effectId !== "string" ||
		!effects.some((effect) => getNodeId(effect) === effectId)
	) {
		return null;
	}
...
export const reduceSetAudio = (payload: unknown, audio: unknown) => {
	const patch = clipSetAudioAction.fn(
		payload,
