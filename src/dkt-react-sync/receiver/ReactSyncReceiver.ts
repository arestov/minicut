...
export class ReactSyncReceiver {
	...
	private attrsReadCache: Map<
		string,
		{ nodeId: string | null; values: Record<string, unknown> }
	>;
	...
	debugDescribeNode(nodeId: string): ReactSyncDebugNode | null {
		...
		return {
			nodeId: node.nodeId,
			modelName:
				node.modelNameKey == null ? null : this.resolveName(node.modelNameKey),
	...
	subscribeRootAttrs(attrNames: readonly string[], listener: Listener) {
		...
		let stopAttrs = this.subscribeNodeAttrs(
			currentRootNodeId,
			attrNames,
			listener,
		);
	...
	subscribeNodeList(
		...
		return this.subscribeNamed(
			this.listSubsByNodeId,
			nodeId,
			relName,
			listener,
		);
		...
		while (cursor < list.length) {
			...
			switch (changeType) {
				case SYRU_UPDATE_ATTRS: {
					...
					const changesLength = Number(
						list[cursor + SYNC_ATTRS_CHANGES_LENGTH] ?? 0,
					);
				...
				case R_UPDATE_TREE_BASE: {
					...
					this.ensureNode(
						...
						(list[cursor + SYNC_TREE_BASE_HIERARCHY_NUM] as number | null) ??
							null,
						(list[cursor + SYNC_TREE_BASE_CONSTR_ID] as
							| number
							| string
							| null) ?? null,
					);
				...
				default: {
					throw new Error(
						`unknown sync update chunk type: ${String(changeType)}`,
					);
				}
			}
		}
