...
const createRuntime = (attrsByNodeId: Record<string, Record<string, unknown>>): ReactScopeRuntime & {
  ...
  const readCachedAttrs = (scope: ReactSyncScopeHandle, attrNames: readonly string[]) => {
    ...
    if (
      cached
      && attrNames.every((attrName) => Object.is(cached[attrName], nextSnapshot[attrName]))
    ) {
      return cached
    }
  ...
  return {
    ...
    subscribeAttrs(scope, _attrNames, listener) {
      const listeners = attrListenersByNodeId.get(scope._nodeId) ?? new Set<() => void>()
